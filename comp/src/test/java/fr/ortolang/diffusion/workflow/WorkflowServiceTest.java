...
public class WorkflowServiceTest {
	...
	private static Logger logger = Logger.getLogger(WorkflowServiceTest.class.getName());
	...
	public static EnterpriseArchive createDeployment() {
		...
		jar.addPackage("fr.ortolang.diffusion.workflow");
		jar.addPackage("fr.ortolang.diffusion.workflow.engine");
		jar.addPackage("fr.ortolang.diffusion.workflow.entity");
		jar.addPackage("fr.ortolang.diffusion.workflow.task");
	...
	public void testCreateWorkflowDefinitionAsUnauthentifiedUser() throws LoginException, KeyAlreadyExistsException, AccessDeniedException, WorkflowServiceException, IOException {
		LoginContext loginContext = AuthenticationLoginContextFactory.createLoginContext("guest", "password");
		loginContext.login();
		try {
			logger.log(Level.INFO, membership.getProfileKeyForConnectedIdentifier());
			workflow.createWorkflowDefinition("K1", new ByteArrayInputStream("".getBytes()));
	...
	@Test
	public void testCreateWorkflowDefinitionAsRootUser() throws LoginException, KeyAlreadyExistsException, AccessDeniedException, WorkflowServiceException, IOException {
		LoginContext loginContext = AuthenticationLoginContextFactory.createLoginContext("root", "tagada54");
		loginContext.login();
		try {
			logger.log(Level.INFO, membership.getProfileKeyForConnectedIdentifier());
			workflow.createWorkflowDefinition("K1", this.getClass().getClassLoader().getResourceAsStream("workflows/HelloWorldProcess.bpmn"));
			List<WorkflowDefinition> defs = workflow.listWorkflowDefinitions();
			
			assertEquals(1, defs.size());
			assertEquals("K1", defs.get(0).getKey());
			
			WorkflowDefinition def = workflow.getWorkflowDefinition("K1");
	...
	@Test
	public void testCreateWorkflowInstanceAsRootUser() throws LoginException, KeyAlreadyExistsException, AccessDeniedException, WorkflowServiceException, IOException {
		LoginContext loginContext = AuthenticationLoginContextFactory.createLoginContext("root", "tagada54");
		loginContext.login();
		try {
			logger.log(Level.INFO, membership.getProfileKeyForConnectedIdentifier());
			workflow.createWorkflowDefinition("K2", this.getClass().getClassLoader().getResourceAsStream("workflows/HelloWorldProcess.bpmn"));
			
			Map<String, Object> params = new HashMap<String, Object>();
			params.put("name", "junit");
			workflow.createWorkflowInstance("K3", "K2", params);
			
			WorkflowInstance ins = workflow.getWorkflowInstance("K3");
