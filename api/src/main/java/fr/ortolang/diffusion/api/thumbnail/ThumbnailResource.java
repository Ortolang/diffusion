...
public class ThumbnailResource {
    ...
    public Response getThumbnailFromKey(@PathParam(value = "key") String key, @QueryParam("size") @DefaultValue("300") int size, @QueryParam("min") @DefaultValue("false") boolean min,
            ...
            throws BrowserServiceException, KeyNotFoundException, OrtolangException, ThumbnailServiceException, UnsupportedEncodingException {
    ...
    private Response getThumbnail(String key, int size, boolean min, boolean login, SecurityContext security, Request request)
            throws BrowserServiceException, KeyNotFoundException, UnsupportedEncodingException {
        try {
            ...
            if (security.getUserPrincipal() == null || security.getUserPrincipal().getName().equals(MembershipService.UNAUTHENTIFIED_IDENTIFIER)) {
                if (login) {
                    UriBuilder builder = UriBuilder.fromPath("/thumbs/" + key);
